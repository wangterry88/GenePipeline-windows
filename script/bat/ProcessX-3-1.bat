...
.\R\R-4.1.1\bin\Rscript.exe .\script\Rscript\3.GWAS+PRS\Step0.Prepare_folder.R %project%
.\R\R-4.1.1\bin\Rscript.exe .\script\Rscript\3.GWAS+PRS\Step1.PatientID_to_TPMI.R %file_name% %output% %ratio% %GWASratio% %project%
  ...
  <body>
    ...
    <script language='javascript' >
        ...
        function pipeText() {
            ...
            var GWASratio = document.getElementById('GWAS-ratio').value;

            var Batch = new ActiveXObject('Scripting.FileSystemObject').GetStandardStream(1);
            close(Batch.WriteLine(file_name+','+output+','+ratio+','+GWASratio));
      }
    </script>
    ...
    <br>
    <br>Please input your GWAS dataset ratio: <input type='text' id="GWAS-ratio" value="0.8" size='5' min="0.5" max="0.9"></input>
    <br>
    <hr>
